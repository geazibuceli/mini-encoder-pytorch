...
class _MultiHeadBase(nn.Module):
    def __init__(self, dimension, heads, dropout=0.0):
        super().__init__()
        if dimension % heads: raise ValueError("Embedding dimension must be divisible by heads.")
        self.dimension, self.heads, self.head_dimension, self.dropout = dimension, heads, dimension // heads, dropout
    ...
    def split(self, values):
        return values.view(values.size(0), values.size(1), self.heads, self.head_dimension).transpose(1, 2)

    def project(self, inputs):
        return self.split(self.q_proj(inputs)), self.split(self.k_proj(inputs)), self.split(self.v_proj(inputs))

    def merge(self, values):
        return values.transpose(1, 2).contiguous().view(values.size(0), values.size(2), self.dimension)
