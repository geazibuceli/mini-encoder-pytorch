...
def evaluate_checkpoint(checkpoint, data_dir, output, batch_size=32, device="cpu"):
    ...
    model, _ = load_model(checkpoint, vocabulary, device, metadata=metadata)
    ...
    report["class_distribution"] = {
        str(label): result["labels"].count(label) for label in sorted(set(result["labels"]))
    }
...
def main():
    ...
    print(
        json.dumps(
            evaluate_checkpoint(args.checkpoint, args.data_dir, args.output, device=args.device),
            indent=2,
        )
    )
